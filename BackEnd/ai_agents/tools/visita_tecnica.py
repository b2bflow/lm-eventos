...
class VisitaTecnica(ITool, FunctionCallMixin):
    name = "visita_tecnica"
    model = "gpt-5.1"
    technical_visit_tag = "visita tecnica"
    ...
    def _block_customer_and_mark_budget(self, customer: dict) -> None:
        ...
        self.customer.update(
            ...
            attributes={
                ...
                "customer_custom_tag": self.technical_visit_tag,
            },
        )

        conversation = self.conversation.get_active_conversation(customer=customer.get("id"))
        if conversation:
            conversation.tag = self.technical_visit_tag
